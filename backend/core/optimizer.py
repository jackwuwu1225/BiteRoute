...
def _is_open(opening_hours: list, arrival_minutes: int) -> bool:
    # 🚨 關鍵修復：因為我們在 routes.py 已經把沒開的店踢掉了
    # 所以在這裡直接一律放行，不去做任何字串切割，避免報錯當機！
    return True
...
def solve_template(
    ...
    for step, candidates in enumerate(candidates_per_node):
        next_beam = []
        ...
        beam = [(p, c) for p, c, s in next_beam[:BEAM_WIDTH]]
        
        if not beam: return TemplateResult()

    best_path = beam[0][0]
    final_score = _mo_score(best_path, total_budget, lat_scale, total_nodes)
    return TemplateResult(score=final_score, assignments=best_path)
